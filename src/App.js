...
const CLASS = {
  CARD: 'border-border bg-primary/25 border-[2px] p-5 rounded-[16px] lg:rounded-[24px] card-bg relative overflow-hidden group hover:border-white transition-colors cursor-pointer h-[172px] md:h-[196px] lg:h-full',
  CARD_TEXT:
...
function App() {
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col justify-center max-lg:pt-12">
      {activePage === 'idle' && (
        <div className="w-full max-w-[1280px] space-y-6 p-3 lg:p-8 mx-auto flex-1 flex flex-col max-h-[1020px]">
          <div className="px-3">
          ...
          <div className="grid lg:grid-cols-3 gap-3 lg:gap-6 flex-1 auto-rows-fr items-stretch">
            {/*CT CARD*/}
            <button className={CLASS.CARD} onClick={() => setActivePage('CT')}>
              ...
              <div className={CLASS.CARD_IMAGE_WRAPPER}>
                <img
                  src="/CT_AGENT.png"
                  className="absolute -translate-x-1/2 left-1/2 max-lg:-top-4 max-lg:w-[180px] lg:bottom-0 z-10 max-h-[648px] group-hover:scale-105 transition-transform"
                />
            ...
            <button className={CLASS.CARD} onClick={() => setActivePage('T')}>
              ...
              <div className={CLASS.CARD_IMAGE_WRAPPER}>
                <img
                  src="/T_AGENT.png"
                  className="absolute -translate-x-1/2  left-1/2 max-lg:-top-16 max-lg:w-[180px] lg:bottom-0 z-10 group-hover:scale-105 transition-transform"
                />
      ...
      {(activePage === 'CT' || activePage === 'T') && (
        <div className="px-3 sm:px-6 lg:px-20  flex w-full  relative">
          <div className="space-y-6 max-lg:w-full relative z-20">
            <div className="flex items-center gap-4 md:gap-6">
              <button
                className="aspect-square w-10 md:w-12 cursor-pointer flex items-center justify-center rounded-[12px] border border-border bg-primary/25"
                onClick={() => setActivePage('idle')}
              >
                <ArrowLeft className="w-6" />
              </button>
              <h1 className="text-[24px] md:text-[36px] font-[900]">
                {activePage} LOADOUT
              </h1>
            </div>
            <div className="grid grid-cols-2 xl:grid-cols-3 gap-3 w-full lg:w-max">
              {{
                CT: CT_SKINS,
                T: T_SKINS,
              }[activePage].map((skin) => (
                ...
                >
                  <div className="w-full md:w-[108px]">
                  ...
                  <div className="space-y-1.5 md:space-y-4">
                    <div>
                      ...
                      <p className="text-lg font-[700]">{skin.finish}</p>
                    </div>
          ...
          <div className="flex-1 max-lg:hidden">
            <div className="relative h-full w-full">
              <div className="absolute z-10 -translate-x-1/2 left-1/2 -translate-y-1/2 top-1/2 flex items-end h-screen">
                <img
                  src={`${activePage}_AGENT.png`}
                  className="min-w-[440px]"
                />
              </div>
              <div className="absolute mix-blend-screen -translate-x-1/2 left-1/2 -translate-y-1/2 top-1/2 flex items-end h-screen">
                <img src={`${activePage}.png`} className="min-w-[800px]" />
              </div>
            </div>
          </div>
        </div>
